...
def test_manual_tag_survives_reimport_that_replaces_tags(conn):
    ...
    db.merge_upsert(
        conn, mk(source="r", source_id="1", title="x", metadata={"tags": ["gaming"]})
    )
    db.set_tags(conn, "r:1", add=["myproject"])
    db.merge_upsert(
        conn,
        mk(
            source="r", source_id="1", title="x", metadata={"tags": ["gaming", "memes"]}
        ),
    )
    md = _md(conn, "r:1")
...
def test_set_auto_tags_stamps_auto_and_keeps_manual_separate(conn):
    db.merge_upsert(conn, mk(source="r", source_id="1", title="x"))
    db.set_tags(conn, "r:1", add=["MyProject"])
    tags = db.set_auto_tags(conn, "r:1", ["gaming", "memes"])
    assert tags == ["gaming", "memes", "myproject"]
    md = _md(conn, "r:1")
    assert md["tags"] == ["gaming", "memes", "myproject"]
    assert md["tags_auto"] == ["gaming", "memes"]
    assert md["tags_manual"] == ["myproject"]


def test_set_auto_tags_can_clear_auto_without_dropping_manual(conn):
    db.merge_upsert(conn, mk(source="r", source_id="1", title="x"))
    db.set_tags(conn, "r:1", add=["MyProject"])
    db.set_auto_tags(conn, "r:1", ["gaming"])
    tags = db.set_auto_tags(conn, "r:1", [])
    assert tags == ["myproject"]
    md = _md(conn, "r:1")
    assert md["tags"] == ["myproject"]
    assert md["tags_manual"] == ["myproject"]
    assert "tags_auto" not in md


def test_set_auto_tags_preserve_base_tags(conn):
    db.merge_upsert(conn, mk(source="youtube", source_id="1", title="x"))
    tags = db.set_auto_tags(conn, "youtube:1", ["science"], preserve_tags=["watch"])
    assert tags == ["watch", "science"]
    md = _md(conn, "youtube:1")
    assert md["tags"] == ["watch", "science"]
    assert md["tags_auto"] == ["science"]


def test_auto_tag_stamp_survives_reimport_that_replaces_tags(conn):
    db.merge_upsert(conn, mk(source="r", source_id="1", title="x"))
    db.set_auto_tags(conn, "r:1", ["gaming"])
    db.merge_upsert(
        conn, mk(source="r", source_id="1", title="x", metadata={"tags": ["fresh"]})
    )
    md = _md(conn, "r:1")
    assert md["tags_auto"] == ["gaming"]
    assert "gaming" in md["tags"]
    assert "fresh" in md["tags"]
