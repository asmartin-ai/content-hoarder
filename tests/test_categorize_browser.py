...
def _add(conn, *, source, sid, title, url="", metadata=None):
    item = models.new_item(source=source, source_id=sid, kind="tab" if source == "firefox" else "story",
...
def test_firefox_pipeline_writes_tags(conn):
    fn = _add(conn, source="firefox", sid="ff|invest", title="Markets wrap: stocks rally",
              url="https://www.bloomberg.com/markets", metadata={"domain": "www.bloomberg.com"})
    res = cat.tag_browser_source(conn, "firefox")
    assert res["source"] == "firefox"
    assert res["tagged"] == 1
    assert res["by_tag"] == {"investing": 1}
    assert parse_metadata(db.get_item(conn, fn)["metadata"])["tags"] == ["investing"]


def test_firefox_pipeline_keyword_path(conn):
    # neutral host -> title keyword drives the bucket
    fn = _add(conn, source="firefox", sid="ff|game", title="My Steam library backlog is huge",
              url="https://blog.example.com/post", metadata={"domain": "blog.example.com"})
...
def test_hackernews_pipeline_writes_tags(conn):
    fn = _add(conn, source="hackernews", sid="hn|def", title="Inside a modern missile defense radar",
...
def test_browser_pipeline_dry_run_does_not_write(conn):
    fn = _add(conn, source="firefox", sid="ff|dry", title="Q3 earnings beat",
              url="https://x.example.com/", metadata={"domain": "x.example.com"})
...
def test_browser_pipeline_skips_already_tagged_unless_retry(conn):
    fn = _add(conn, source="hackernews", sid="hn|skip", title="Inside a missile defense radar",
              url="https://www.defensenews.com/x", metadata={"tags": ["defense"]})
...
def test_browser_pipeline_no_match_leaves_untagged(conn):
    fn = _add(conn, source="firefox", sid="ff|none", title="How to bake sourdough bread",
              url="https://example.com/recipe", metadata={"domain": "example.com"})
    res = cat.tag_browser_source(conn, "firefox")
    assert res["tagged"] == 0
...
def test_browser_pipeline_never_introduces_category(conn):
    ...
    fn = _add(conn, source="firefox", sid="ff|nocat", title="stocks rally",
              url="https://x.example.com/", metadata={"domain": "x.example.com"})
    cat.tag_browser_source(conn, "firefox")
