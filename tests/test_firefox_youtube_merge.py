...
def _seed_library(conn):
    _seed(
        ...
        [
            ...
            _firefox_yt(
                "ff_dup",
                "https://www.youtube.com/watch?v=DUPvid000_1&list=WL",
                "(5) messy dup title - YouTube",
                window="3",
                pinned=1,
            ),  # dupe
            _firefox_yt(
                "ff_orphan",
                "https://youtu.be/ORPvid00002",
                "(9) Orphan Title - YouTube",
                window="3",
            ),  # orphan
            _firefox_yt(
                "ff_done",
                "https://www.youtube.com/watch?v=DONvid00003",
                "Watched One - YouTube",
                status="done",
            ),  # orphan, already triaged
            ...
            dict(
                ...
                kind="tab",
                title="Mod",
                url="https://modrinth.com/mod/x",
                metadata={"domain": "modrinth.com"},
            ),
        ],
    )
...
def test_migrate_apply(tmp_db):
    ...
    assert (
        res["dupes"] == 1 and res["orphans"] == 2 and res["firefox_rows_removed"] == 3
    )
    ...
    wl = _item(conn, "youtube:DUPvid000_1")
    ...
    orp = _item(conn, "youtube:ORPvid00002")
    ...
    done_row = _item(conn, "youtube:DONvid00003")
    assert done_row["status"] == "done"
    # processed_utc must be preserved so the item counts in weekly-stats and undo works
    assert (
        done_row["processed_utc"] is None
    )  # seeded without one — still None, not forced
...
def test_migrate_preserves_processed_utc(tmp_db):
    ...
    conn.execute(
        "UPDATE items SET processed_utc=1748000000 WHERE fullname='firefox:ff_triaged'"
    )
    conn.commit()
    fy.migrate(conn, apply=True)
    promoted = _item(conn, "youtube:TRGvid00001")
...
def test_migrate_same_video_in_two_tabs_collapses_to_one(tmp_db):
    ...
    _seed(
        conn,
        [  # same video open in two tabs (slightly different URLs -> two firefox rows)
            _firefox_yt(
                "ff_a",
                "https://www.youtube.com/watch?v=SAMEvid0001&t=10",
                "Tab A - YouTube",
            ),
    ...
    assert (
        db.get_item(conn, "firefox:ff_a") is None
        and db.get_item(conn, "firefox:ff_b") is None
    )
