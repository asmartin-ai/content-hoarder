...
def _seed_library(conn):
    _seed(conn, [
        ...
        dict(source="youtube", source_id="DUPvid000001", kind="video", title="Real WL Title",
        ...
        _firefox_yt("ff_dup", "https://www.youtube.com/watch?v=DUPvid000001&list=WL",
                    "(5) messy dup title - YouTube", window="3", pinned=1),       # dupe
        _firefox_yt("ff_orphan", "https://youtu.be/ORPvid000002",
                    "(9) Orphan Title - YouTube", window="3"),                    # orphan
        _firefox_yt("ff_done", "https://www.youtube.com/watch?v=DONvid000003",
...
def test_migrate_dry_run_makes_no_changes(tmp_db):
    ...
    assert db.get_item(conn, "youtube:ORPvid000002") is None
...
def test_migrate_apply(tmp_db):
    ...
    wl = db.get_item(conn, "youtube:DUPvid000001")
    ...
    assert wmd["open_in_firefox"] is True and wmd["via_firefox"] is True
    ...
    orp = db.get_item(conn, "youtube:ORPvid000002")
    ...
    assert orp["url"] == "https://youtu.be/ORPvid000002"
    assert omd["open_in_firefox"] is True and omd["firefox_window"] == "3"
    assert omd["thumbnail"].endswith("/ORPvid000002/hqdefault.jpg")

    # ORPHAN with prior triage keeps its status (a 'done' tab stays done)
    assert db.get_item(conn, "youtube:DONvid000003")["status"] == "done"
    ...
    assert {h["fullname"] for h in hits} == {
        "youtube:DUPvid000001", "youtube:ORPvid000002", "youtube:DONvid000003"}
...
def test_migrate_same_video_in_two_tabs_collapses_to_one(tmp_db):
    ...
    _seed(conn, [  # same video open in two tabs (slightly different URLs -> two firefox rows)
        _firefox_yt("ff_a", "https://www.youtube.com/watch?v=SAMEvid00001&t=10", "Tab A - YouTube"),
        _firefox_yt("ff_b", "https://youtu.be/SAMEvid00001", "Tab B - YouTube"),
    ])
    res = fy.migrate(conn, apply=True)
    ...
    assert db.get_item(conn, "youtube:SAMEvid00001") is not None
    hits = db.search_items(conn, open_in_firefox=True, limit=50)
    assert [h["fullname"] for h in hits] == ["youtube:SAMEvid00001"]
