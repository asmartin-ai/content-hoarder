...
def test_recover_one_default_does_not_archive_media(tmp_db):
    ...
    res = _recover_result(archival.recover_one(conn, "reddit:t3_gone", providers=[]))
    assert res["bytes_archived"] == 0
    assert "archive_today" not in res
    md = json.loads(_item(conn, "reddit:t3_gone")["metadata"])
...
def test_recover_one_skips_archive_today_when_media_live(tmp_db):
    ...
    res = _recover_result(
        archival.recover_one(
            conn,
            "reddit:t3_live",
            media_providers=[Boom(UA)],
            fetch_bytes=lambda *a, **k: (None, ""),
            apply_bytes=True,
        )
    )
...
def test_provider_extracts_og_image_and_inline_imgs():
    html = (
        '<meta property="og:image" content="https://archive.ph/a.jpg">'
    ...
    res = p.recover_media(
        {
            "metadata": {
                "media_url": "https://i.redd.it/orig.jpg",
                "media_status": "gone",
            }
        }
    )
...
def test_provider_cloudflare_403_skips_silently():
    ...
    res = p.recover_media(
        {"metadata": {"media_url": "https://i.redd.it/x.jpg", "media_status": "gone"}}
    )
...
def test_provider_gallery_looks_up_each_frame():
    ...
    res = p.recover_media(
        {
            "metadata": {
                "media_url": "https://i.redd.it/main.jpg",
                "media_status": "gone",
